...
def retrieve_knowledge(
    ...
    return "\n".join(
        text
        for key, text in WEATHER_KNOWLEDGE
        if key in q
    )[:2500]
...
async def geocode_reverse(
    ...
    try:
        ...
        raise HTTPException(
            502,
            detail=f"Reverse geocoding error: {exc}"
        )
...
def local_answer(
    ...
    # Detect tomorrow / future questions

    tomorrow = (
        "tomorrow" in message
        or "next day" in message
    )
    ...
    today = (
        ...
        or "currently" in message
    )

    # Tomorrow's forecast

    if tomorrow and daily:
        ...
        if len(dates) > 1:
            ...
            if tomorrow_code in [
                ...
                tomorrow_condition = "rainy"

            elif tomorrow_code in [
                95,
                96,
                99
            ]:

                tomorrow_condition = "stormy"

            elif tomorrow_code not in [
                0,
                None
            ]:

                tomorrow_condition = "unsettled"

            if req.language == "en":

                if (
                    "rain" in message
                    or "raining" in message
                    or "precipitation" in message
                ):

                    return (
                        f"Tomorrow in {place}, the rain "
                        f"probability is about "
                        f"{tomorrow_probability}% with "
                        f"{tomorrow_rain} mm of precipitation."
                    )
                ...
                if (
                    ...
                    return (
                        f"Tomorrow in {place}, the temperature "
                        f"is expected to range from "
                        f"{tomorrow_temp_min}°C to "
                        f"{tomorrow_temp_max}°C."
                    )

                if "wind" in message:

                    return (
                        f"Tomorrow in {place}, the maximum "
                        f"wind speed is expected to be about "
                        f"{tomorrow_wind} km/h."
                    )
                ...
                return (
                    ...
                    f"{tomorrow_condition}. "
                    f"Rain probability is about "
                    f"{tomorrow_probability}%."
                )

    # Current / today questions
    ...
    if req.language == "en":
        ...
        if (
            ...
            return (
                f"In {place}, current precipitation is "
                f"{rain} mm."
            )
        ...
        if "temperature" in message:
            ...
            return (
                f"In {place}, the current temperature "
                f"is about {temp}°C."
            )
        ...
        if "wind" in message:
            ...
            return (
                f"In {place}, the current wind speed "
                f"is about {wind} km/h."
            )

        return (
            f"In {place}, it is about {temp}°C and "
            f"currently {condition}. Precipitation is "
            f"{rain} mm and wind is around {wind} km/h."
        )
...
Do not repeat the same current-weather answer for every question.

If the user asks about tomorrow, use tomorrow's forecast data.
If the user asks about today or current weather, use current weather data.
If the user asks about rain, answer about precipitation or rain probability.
If the user asks about temperature, answer about temperature.
If the user asks about wind, answer about wind.
If the user asks about a forecast, use the supplied forecast data.
If the user asks about farming, use the weather data and farming guidance.
If the user asks about travel, use the weather data and travel guidance.
...
2. Did you answer the specific question?
3. Did you accidentally use English sentences?
4. Did you mix languages?
